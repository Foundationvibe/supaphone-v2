...
Deno.serve(async request => {
    ...
    try {
        ...
        if (!requesterClientId || !requesterClientSecret || !requesterClientType || !targetClientId) {
            return jsonResponse(
                {
                    ok: false,
                    error: "requesterClientId, requesterClientSecret, requesterClientType, and targetClientId are required."
                },
                400
            );
        }
        ...
        if (requesterClientId === targetClientId) {
            return jsonResponse({ ok: false, error: "Cannot remove pairing with the requester identity itself." }, 400);
        }
        ...
        if (requesterClientQuery.error) {
            return internalServerError("remove-paired-device:select-requester-client", requesterClientQuery.error);
        }
        ...
        if (linkCheck.error) {
            return internalServerError("remove-paired-device:select-pair-link", linkCheck.error);
        }
        if (!linkCheck.data?.id) {
            return jsonResponse({ ok: false, error: "No active pairing found to remove." }, 404);
        }

        const revokeResult = await supabase
            .from("pair_links")
            .update({ status: "revoked" })
            .eq("id", linkCheck.data.id)
            .select("id")
            .maybeSingle();

        if (revokeResult.error) {
            return internalServerError("remove-paired-device:update-pair-link", revokeResult.error);
        }
        if (!revokeResult.data?.id) {
            return jsonResponse({ ok: false, error: "Pair link not found." }, 404);
        }

        return jsonResponse({
            ok: true,
            pairLinkId: linkCheck.data.id,
            removedClientId: targetClientId
        });
    } catch (error) {
        return internalServerError("remove-paired-device:unhandled", error);
    }
});
